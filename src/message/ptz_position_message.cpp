...
PTZPositionNotifyMessage::PTZPositionNotifyMessage(const std::string &device_id, const std::string &reason)
    : MessageBase() {
    device_id_ = device_id;
    reason_ = reason;
...
bool PTZPositionNotifyMessage::load_detail() {
    auto root = xml_ptr_->RootElement();
    from_xml_element(pan_, root, "Pan");
    from_xml_element(tilt_, root, "Tilt");
    from_xml_element(zoom_, root, "Zoom");
...
bool PTZPositionNotifyMessage::parse_detail() {
    auto root = xml_ptr_->RootElement();
    if (pan_)
        new_xml_element(pan_, root, "Pan");
    if (tilt_)
